package anifire.cc_theme_lib
{
   ...
   public class CcCharacter
   {
      ...
      public function deserialize(param1:XML, param2:UtilHashArray) : void
      {
         ...
         {
            if((_loc8_ = CcComponent.getComponentThumbTypeFromXml(_loc3_)) == CcLibConstant.COMPONENT_TYPE_BODYSHAPE)
            {
         ...
         {
            if((_loc5_ = new CcColor()).deserialize(_loc3_,this.currentTheme,this))
            {
         ...
         for each(_loc3_ in param1.child(CcLibrary.XML_NODE_NAME))
         {
            ...
            if(_loc12_ = this.currentTheme.getComponentThumbByInternalId(CcComponentThumb.generateInternalId(_loc3_.@type,_loc3_.@component_id)))
            {
